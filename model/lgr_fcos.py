...
from .backbone.lgr_backbone import cbrt
import torch.nn as nn
from .loss import GenTargets,LOSS,coords_fmap2orig
import torch
from .config import DefaultConfig
from .network_blocks import Dilated
...
class FCOS(nn.Module):
    ...
    def __init__(self,config=None):
        ...
        self.backbone=cbrt(pretrained=config.pretrained,if_include_top=False)
    ...
    def forward(self,x):
        ...
        all_P[0] = self.rfrm(all_P[0])
        all_P[1] = self.rfrm(all_P[1])
        all_P[2] = self.rfrm(all_P[2])
