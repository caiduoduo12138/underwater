...
def compute_cls_loss(preds,targets,mask):
    ...
    for batch_index in range(batch_size):
        ...
        # loss.append(QFL(pred_pos, target_pos).view(1))
...
def compute_reg_loss(preds,targets,mask,mode='giou'):
    ...
    for batch_index in range(batch_size):
        ...
        if mode=='iou':
            ...
            # loss.append(2.0*giou_loss(pred_pos, target_pos).view(1) + 0.25*DFL(pred_pos, target_pos).view(1))
        elif mode=='diou':
            loss.append(diou_loss(pred_pos,target_pos).view(1))
        elif mode=='ciou':
            loss.append(ciou_loss(pred_pos,target_pos).view(1))
        else:
            raise NotImplementedError("reg loss only implemented ['iou','giou','diou','ciou']")
    return torch.cat(loss,dim=0)/num_pos#[batch_size,]
...
def diou_loss(preds, targets, eps=1e-7):
    ...
    # enclose area
    enclose_x1y1 = torch.max(preds[:, :2], targets[:, :2])
    ...
    b1_x1, b1_y1 = preds[:, 0], preds[:, 1]
    b1_x2, b1_y2 = preds[:, 2], preds[:, 3]
    b2_x1, b2_y1 = targets[:, 0], targets[:, 1]
    b2_x2, b2_y2 = targets[:, 2], targets[:, 3]
...
def QFL(preds,targets,beta=2.0):
    bce = torch.nn.BCEWithLogitsLoss(reduction="none")
    pt = preds.sigmoid()
    loss = bce(preds, targets) * ((targets - pt).abs().pow(beta))
    return loss.sum()

def DFL(predicts, targets):
    ...
    loss = ce(predicts, disl) * wl + ce(predicts, disr) * wr
    return loss.sum()
