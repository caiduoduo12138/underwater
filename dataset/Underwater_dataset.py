...
class UnderwaterDataset(CocoDetection):
    CLASSES_NAME = ('holothurian', 'echinus', 'scallop', 'starfish')
    # CLASSES_NAME = ('seacucumber', 'seaurchin', 'scallop')
    def __init__(self,imgs_path,anno_path,resize_size=[800,1333],mode=None,is_train = True, transform=None):
        super().__init__(imgs_path,anno_path)

        assert ((type(resize_size[1])==int) and (mode==None)) or ((type(resize_size[1]==tuple)) and  (mode=='range' or  mode== 'value')), \
            "multi-scale error, mode is not exisitance! you should set input size as [(, ), (, )] and mode in ['range', 'value'] or set mode=None and the input size as[ , ]!"
        ...
        # self.mean=[0.242100, 0.552255, 0.213055]
        # self.std=[0.092960, 0.150676, 0.068132]
    ...
    def __getitem__(self,index):
        ...
        if  self.train:
            ...
            '''
             TTA will be support soon!
            '''
        #img, boxes = self.preprocess_img_boxes(img, boxes, self.resize_size)#single scale test!
        img=np.array(img)
        if self.mode != None:

            img, boxes = self.preprocess_img_boxes(img, boxes, self._resize(self.resize_size))
        else:
    ...
    def _resize(self,_size):
        if self.mode=='range':
            assert len(_size) == 2, "invalid input size, the length of list should be 2, and set like [(), ()]!"
            img_scale_long = [max(s) for s in _size]
            img_scale_short = [min(s) for s in _size]
            ...
            img_scale = [short_edge, long_edge]

        elif self.mode == 'value':
            '''
            randomly select one image size from list
            '''
            assert len(_size) >= 2, "invalid input size, the length of list should be greater than 2, and set like [(), ()]!"
            index = np.random.randint(0, len(_size)-1)
            img_scale = _size[index]

        else:
            raise KeyError("error mode, please choose in [range, value]!")
        # print(img_scale)
        return img_scale
...
if __name__=="__main__":
    ...
    dataset=UnderwaterDataset("/home/cai/data/experiment/images","/home/cai/data/experiment/train.json")
